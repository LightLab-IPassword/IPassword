...
use ipassword_shared::config::{Config, Hotkey, MOD_NOREPEAT};
...
use winapi::um::winuser::{
    ...
    TPM_RIGHTBUTTON, WM_APP, WM_COMMAND, WM_CONTEXTMENU, WM_DESTROY, WM_HOTKEY, WM_LBUTTONUP,
...
const WM_CLOSE_REQUEST: UINT = 0x0010; // WM_CLOSE
...
fn on_capture() {
    let _ = with_state(|st| {
        if popup::is_running(&mut st.child) {
            return;
        }
        ...
        match popup::spawn("popup-capture", target, Some(data)) {
...
fn on_paste() {
    let _ = with_state(|st| {
        ...
        match popup::spawn("popup-paste", target, None) {
            Ok(child) => st.child = Some(child),
            Err(e) => win::message_box(
                "IPassword",
