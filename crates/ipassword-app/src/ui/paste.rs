//! The "paste a password" window (Alt+V). Sign in, name the app, click what you want typed.
...
use crate::core_dll::Vault;
use crate::paths;
use crate::records::RecordData;
use crate::win::{self, TypeJob};
...
pub fn run(target: usize) {
    ...
    run_app(
        title,
        popup_options(title, [480.0, 560.0]),
...
impl PasteApp {
    ...
    fn lookup(&mut self) {
        ...
        match vault.get_app(&self.app) {
            ...
            Ok(_) => {
                self.error = Some(format!("Nothing saved for \"{}\".", self.app.trim()));
            }
    ...
    /// Already unlocked, but the app name didn't match anything: just ask for another name.
    fn draw_lookup(&mut self, ui: &mut egui::Ui) {
        let pal = self.pal;
        ui.heading("Unlocked");
        ui.label(egui::RichText::new("Which app do you want?").color(pal.muted));
        ui.add_space(2.0);
        ...
        ui.add_space(4.0);
        status_line(ui, &pal, working, &self.error);
        if primary_button(ui, &pal, "Find", !working).clicked() || submit {
            self.request();
        }
    }

    fn draw_pick(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        let pal = self.pal;
