...
pub fn run() {
    ...
    if win::daemon_window().is_none() {
        win::start_daemon();
    }
    run_app(
        "IPassword",
        window_options("IPassword", [540.0, 640.0]),
        ManageApp::new(),
    );
}
...
impl ManageApp {
    fn new() -> Self {
        ...
        let (core_version, core_error) = match core_dll::core() {
        ...
        ManageApp {
            ...
            daemon_running: win::daemon_window().is_some(),
    ...
    fn save_hotkeys(&mut self) {
        ...
        if let Err(e) = config.save(&paths::config_path()) {
            self.fail(format!("Couldn't save config.json: {e}"));
            return;
        }
        self.config = config;
        win::restart_daemon();
        self.daemon_running = win::daemon_window().is_some();
    ...
    fn draw_home(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(format!(
            "2. Click into a login box and press {} to paste it.",
    ...
    fn draw_settings(&mut self, ui: &mut egui::Ui) {
        ...
        if ui
            .checkbox(&mut self.autostart, "Start IPassword when I sign in to Windows")
            .changed()
        {
            if let Err(e) = win::set_autostart(self.autostart) {
...
corner radius, font size) sit next to the program. Edit them and restart IPassword.",
            )
            .color(pal.muted),
        );
        if ui.button("Open program folder").clicked() {
            win::open_folder(&paths::exe_dir());
        }
