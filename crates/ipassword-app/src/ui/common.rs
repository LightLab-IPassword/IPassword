...
use crate::core_dll::{self, CoreError};
use crate::paths;
...
/// A small pop-up window that appears on top of everything (the Alt+C / Alt+V windows).
...
pub fn run_app<A: eframe::App + 'static>(title: &str, options: eframe::NativeOptions, app: A) {
    ...
    if let Err(e) = result {
        win::message_box("IPassword", &format!("Couldn't open the window: {e}"), true);
    }
}
...
/// Loads the core DLL or shows why it couldn't be loaded.
pub fn load_core() -> Option<&'static core_dll::Core> {
    match core_dll::core() {
        Ok(core) => Some(core),
        Err(message) => {
            win::message_box("IPassword", &message, true);
            None
        }
    }
}
...
pub fn describe_error(e: &CoreError) -> String {
    match e.code {
        core_dll::WRONG_PASSWORD => "Wrong master password.".to_string(),
        core_dll::USER_NOT_FOUND => {
            "No such user. Create an account in the IPassword window first.".to_string()
        }
        core_dll::USER_EXISTS => "That username already exists.".to_string(),
        core_dll::CORRUPT => "A vault file looks damaged or tampered with.".to_string(),
